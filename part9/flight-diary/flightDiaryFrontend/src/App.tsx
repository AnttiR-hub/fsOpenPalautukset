...
const App = () => {
  ...
  const weatherOptions: WeatherOption[] = Object.values(Weather).map(elm => ({
    value: elm, label: elm.toString()
  }));
  ...
  const visibilityOptions: VisibilityOption[] = Object.values(Visibility).map(elem => ({
    value: elem, label: elem.toString()
  }));
  ...
  const onVisibilityChange = (event: SelectChangeEvent<string>) => {
    ...
    if ( typeof event.target.value === "string") {
      const value = event.target.value;
      const visibility = Object.values(Visibility).find(elem => elem.toString() === value);
